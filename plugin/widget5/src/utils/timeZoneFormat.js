// Shared formatting for the app-wide CKT/UTC display toggle (timeDisplayZone
// state, lifted to Home.jsx). Both zones this app supports are fixed-offset
// (Pacific/Rarotonga is UTC-10 year-round, no DST), so the datetime-local
// round-trip below is exact — no DST-transition ambiguity to handle.

export const TZ_LABELS = { UTC: 'UTC', 'Pacific/Rarotonga': 'CKT' };

export function tzLabel(timeZone) {
  return TZ_LABELS[timeZone] || timeZone;
}

// Formats a Date for display in `timeZone`, e.g. "21/07/2026 14:05 CKT".
