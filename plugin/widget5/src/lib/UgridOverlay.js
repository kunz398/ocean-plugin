...
export class UgridOverlay {
  ...
  async _loadMetadata() {
    ...
    const extraDefs = Array.isArray(this.config.pointVariables) ? this.config.pointVariables : [];
            ...
            for (const [level, segs] of byLevel) {
              ...
              segs.forEach((seg, i) => {
                ...
                labelData.push({
                  ...
                  text: `${level.toFixed(contourCfg.labelDecimalPlaces ?? 1)} ${contourCfg.units ?? this.config.units ?? 'm'}`,
                });
              });
            }
      ...
      if (numDirs && showDirectionArrows && currentZoom >= minArrowZoom) {
        ...
        layers.push(new IconLayer({
          ...
          getColor: (d) => {
            ...
            // Floor at 150 so even low-magnitude arrows remain legible on satellite imagery.
            const alpha = Math.round(150 + 90 * tc);
            // 'band' mode: tint the halo with the wave-height band colour.
            ...
            // 'neutral' mode: the SVG icon has fill=black (arrow body) and stroke=white (halo).
            // deck.gl IconLayer multiplies white pixels by getColor, so this controls the halo.
            // White halo + black arrow body reads clearly on both satellite and raster backgrounds.
            const [nr, ng, nb] = this.config.arrowColor ?? [20, 35, 50];
  ...
  async getTimeseriesAtPoint(lng, lat) {
    ...
    if (this.config.ugridApiBase) {
      const apiUrl = `${this.config.ugridApiBase}/wave/ugrid/timeseries?lon=${lng}&lat=${lat}`;
      ...
      try {
        ...
        console.warn('[UgridOverlay] API returned', resp.status, '— falling back to zarr');
      } catch (e) {
        clearTimeout(timeoutId);
        if (e.name !== 'AbortError') {
          console.warn('[UgridOverlay] API timeseries failed, falling back to zarr:', e.message);
        }
      }
    }

    // Fallback: direct zarr chunk fetching (slow — ~2700 requests for 15 variables)
