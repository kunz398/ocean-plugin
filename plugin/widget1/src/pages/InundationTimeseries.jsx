...
import { seaLevelHeightM } from '../lib/NiueSuitabilityOverlay';
...
function InundationTimeseries({ timeseries, categories, rangeWindow, isDarkMode, currentSliderDate, onTimeSelect, seaLevelTimeseries }) {
  ...
  const lastPlotSizeRef = useRef({ width: 0, height: 0 });
  ...
  const syncPlotSize = useCallback(() => {
    ...
    const nextHeight = Math.max(180, height);
    setPlotHeight((current) => current === nextHeight ? current : nextHeight);

    const graphDiv = plotlyDivRef.current;
    const last = lastPlotSizeRef.current;
    if (graphDiv && (last.width !== width || last.height !== height)) {
      lastPlotSizeRef.current = { width, height };
  ...
  useEffect(() => {
    ...
    const obs = typeof ResizeObserver !== 'undefined'
      ? new ResizeObserver(schedulePlotResize)
      : null;
    obs?.observe(el);
  ...
  // Trim to the operational range: dry → niu-maximum (inclusive).
  ...
  const realCategories = useMemo(() => {
    ...
    const maxIdx = categories.findIndex(c => c.id === 'niu-maximum');
  ...
  const tideTrace = useMemo(() => {
    if (!showTide) return null;
    const steps = seaLevelTimeseries?.data?.timesteps;
    if (!Array.isArray(steps) || !steps.length || !times.length) return null;
    const startMs = times[0].getTime();
    const endMs = times[times.length - 1].getTime();
    const points = steps
      .map((step) => {
        const timeValue = step.valid_time_utc ?? step.valid_time ?? step.time ?? step.time_utc;
        const ms = timeValue ? new Date(timeValue).getTime() : NaN;
        const height = seaLevelHeightM(step);
        return Number.isFinite(ms) && Number.isFinite(height) ? { time: new Date(ms), height } : null;
      })
      .filter((point) => point && point.time.getTime() >= startMs && point.time.getTime() <= endMs);
    if (!points.length) return null;
    return {
      type: 'scatter',
      x: points.map((point) => point.time),
      y: points.map((point) => point.height),
      yaxis: 'y2',
      mode: 'lines',
      name: 'Tide',
      showlegend: true,
      line: { color: isDarkMode ? '#7dd3fc' : '#0284c7', width: 1.5, dash: 'dash', shape: 'spline', smoothing: 0.6 },
      hovertemplate:
        '<span style="font-size:12px"><b>%{x|%b %d %H:%M UTC}</b></span><br>' +
        'Tide: <b>%{y:.2f} m</b><extra></extra>',
    };
  }, [showTide, seaLevelTimeseries?.data?.timesteps, times, isDarkMode]);

  const hasTideData = Array.isArray(seaLevelTimeseries?.data?.timesteps)
    && seaLevelTimeseries.data.timesteps.length > 0;
  ...
  }, [times, depths, peakStyle, isDarkMode, tideTrace]);
  ...
  const { thresholdShapes, thresholdAnnotations } = useMemo(() => {
    ...
    realCategories.forEach((cat, idx) => {
      if (idx === 0 || idx === n - 1) return;   // skip dry (0 m) and upper bound (niu-maximum)
          ...
          {hasTideData && (
            <label className="inundation-timeseries__toggle">
              <input
                type="checkbox"
                checked={showTide}
                onChange={(event) => setShowTide(event.target.checked)}
              />
              Show tide
            </label>
          )}
      ...
      <div className="inundation-timeseries__chart" ref={chartRef}>
        <Plot
          ...
          layout={{
            autosize: true,
            height: plotHeight,
            margin: { l: 44, r: tideTrace ? 54 : (showThresholdLabels ? 64 : 18), t: 8, b: 36 },
            ...
            yaxis2: {
              ...
              showgrid: false,
              linecolor: 'transparent',
              zerolinecolor: isDarkMode ? 'rgba(125,211,252,0.25)' : 'rgba(2,132,199,0.25)',
              zeroline: true,
              ticks: 'outside', ticklen: 4, tickcolor: 'transparent',
            },
            showlegend: Boolean(tideTrace),
            legend: {
              x: 1,
              xanchor: 'right',
              y: 1,
              ...
              font: { color: tick, size: 10, family: 'Inter, system-ui, sans-serif' },
            },
            clickmode: 'event',
            dragmode: false,
            shapes: allShapes,
